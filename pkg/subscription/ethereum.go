...
func NewMempoolSubscription(client *clients.EthereumClient, toAddress string) (*MempoolSubscription, error) {
...
func (s *MempoolSubscription) handleTransactions(ctx context.Context) {
	for {
		...
		case tx := <-s.txChan:
			...
			tx, isPending, err := ethClient.TransactionByHash(ctx, tx.Hash())
			if err != nil {
